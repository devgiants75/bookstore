...
import { useSelectProductStore, useUserStore } from '../../stores';
...
export default function DetailPage() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:4080/api/book/${productSeq}`).then(response => {
      const { data } = response;
      const { result } = data;
      console.log(result);
      ...
      <Box display="flex" justifyContent="center">
        ...
        >
                ...
                <div>
                  <ButtonGroup>
                    <Box border={1} display="flex">
                      <Button
                        onClick={() => {
                          setCartProductAmount(
                            Math.max(cartProductAmount - 1, 0),
                          );
                        }}
                      >
                      ...
                      <Box pl={3}>
                        <Badge badgeContent={cartProductAmount} />
                      </Box>
                      <Button
                        onClick={() => {
          ...
          <Box display="flex" justifyContent="space-between" padding={1}>
            ...
            <Box>
              <Typography padding={1}>{productSalesPrice + 3000}원</Typography>
            </Box>
          </Box>
              ...
              <TabPanel value="3">
                <Box padding={2}>
                  ...
                  <Typography fontSize={18}>
                    배송방법 택배 / CJ 대한통운 네이버페이 구매 후 반품/교환 시
                    우체국 택배로 자동 수거되지 않으며, 웅진책방 전용 택배사인
                    CJ대한통운으로 수거처리 됩니다.
                  </Typography>
                </Box>
                <Box padding={2}>
                  <Typography
                    ...
                    borderColor="#F0A500"
                  >
                    배송비 기본 배송비 3,000원 (제주/도서산간 지역의 경우
                    추가비용발생) 30,000원 이상 무료 (단, 네이버페이와 같은
                    간편결제시 제주/도서산간 지역의 경우 추가비용이 발생합니다.)
                  </Typography>
                </Box>
                <Box padding={2}>
                  <Typography fontSize={25} paddingBottom={2}>
                  ...
                  <Typography
                    ...
                    borderColor="#F0A500"
                  >
                    반품/교환 방법 마이페이지 반품/교환 신청 및 조회,
                    고객센터(1670-3780)로 유선 연락 반품/교환 가능 기간
                    출고변심반품의 경우 수령 후 7일 이내 재화등의 내용이
                    표시·광고의 내용과 다르거나 계약내용과 다르게 이행된
                    경우에는 그 사실을 안 날 또는 알 수 있었던 날부터 30일
                    이내에 청약철회가 가능 (단, 수령일로 부터 최대 3개월)
                  </Typography>
                </Box>
                <Box padding={2}>
                  <Typography fontSize={25} paddingBottom={2}>
                  ...
                  <Typography
                    ...
                    borderColor="#F0A500"
                  >
                    고객의 단순변심 및 구매 착오로 인한 상품 반송비용은 고객
                    부담
                  </Typography>
                </Box>
                <Box padding={2}>
                  ...
                  <Typography
                    ...
                    borderColor="#F0A500"
                  >
                    소비자에게 책임이 있는 사유로 재화등이 멸실되거나 훼손된
                    경우. 다만, 재화등의 내용을 확인하기 위하여 포장 등을 훼손한
                    경우는 제외 소비자의 사용, 포장 개봉에 의해 상품 등의 가치가
                    현저히 감소한 경우 복제가 가능한 상품 등의 포장을 훼손한
                    경우 디지털 컨텐츠인 eBook, 오디오북 등을 1회 이상
                    다운로드를 받았을 경우 시간의 경과에 의해 재판매가 곤란한
                    정도로 가치가 현저히 감소한 경우 전자상거래 등에서의
                    소비자보호에 관한 법률이 정하는 소비자 청약철회 제한 내용에
                    해당되는 경우
                  </Typography>
                </Box>
                <Box padding={2}>
                  ...
                  <Typography fontSize={18}>
                    상품의 불량에 의한 반품, 교환, A/S, 환불, 품질보증 및
                    피해보상 등에 관한 사항은 소비자분쟁해결기준(공정거래위원회
                    고시)에 준하여 처리됨
                  </Typography>
                </Box>
                <Box padding={2}>
                  <Typography fontSize={18}>환불 지연에 따른 배상</Typography>
                  <Typography borderBottom={1} borderColor="#F0A500">
                    대금 환불 및 환불 지연에 따른 배상금 지급 조건, 절차 등은
                    전자상거래 등에서의 소비자 보호에 관한 법률에 따라 처리
                  </Typography>
                </Box>
              </TabPanel>
            </Box>
