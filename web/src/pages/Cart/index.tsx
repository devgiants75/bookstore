...
export default function Cart() {
  ...
  const deleteCartList = (cartId: number) => {
    ...
    axios
      .post('http://localhost:4080/api/cart/cartDelete', data, {
  ...
  const saveCartList = () => {
    ...
    axios
      .post('http://localhost:4080/api/cart/cartAmountUpdate', data, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(response => {
        const { data } = response;
  ...
  const getCartList = () => {
    ...
    axios
      .post(
        'http://localhost:4080/api/cart/showInCart',
        {},
        {
          headers: {
  ...
  const addAmount = (cartId: number, num: number) => {
    ...
    cartList.forEach(cartItem => {
      if (
        selectCart.findIndex((element: any) => element === cartItem.cartId) !==
        -1
      ) {
        total += cartItem.cartProductPrice * cartItem.cartProductAmount;
        totalCount += cartItem.cartProductAmount;
  ...
  return (
    <div>
            ...
            {cartList.map(cartItem => (
              <tr>
                <td>
                  <input
                    type="checkBox"
                    onChange={() => setSelectCartItem(cartItem.cartId)}
                  />
                </td>
                ...
                <td>
                  <button
                    ...
                    onClick={() => addAmount(cartItem.cartId, 1)}
                  >
                    △
                  </button>{' '}
                  {cartItem.cartProductAmount} 권{' '}
                  <button
                    onClick={() => addAmount(cartItem.cartId, -1)}
                    type="button"
                  >
                ...
                <td>
                  {cartItem.cartProductPrice * cartItem.cartProductAmount}원
                </td>
                <td>
                  <button
                    onClick={() => deleteCartList(cartItem.cartId)}
                    type="button"
                  >
                    ❎
                  </button>
      ...
      <div className="cart-total-area">
        <div className="inner">
          ...
          <div className="cart-box-img">
            <img
              src="https://www.woongjinbooks.com/static/pc/images/sub/review_order01.png"
              alt="plus"
            />
          </div>
          <div className="cart-box-shipping">
            <dl>
              ...
              <dd>
                <p>
                  <span id="delivery-price" className="price-black">
                    <strong>{expectedPrice}</strong>원
                  </span>
                </p>
                <p>
                  <span className="note-txt">(30,000원 이상 무료배송)</span>
                </p>
              </dd>
            </dl>
          </div>
          <div className="cart-box-img">
            <img
              src="https://www.woongjinbooks.com/static/pc/images/sub/review_order02.png
                    "
              alt="equlas"
            />
          </div>
          <div className="cart-box-total">
            <dl>
              <dt>총 결제 예상 금액</dt>
              <dd>
                <p id="total-payment-price" className="txt-red">
                  <strong>{expectedPrice}</strong>원
                </p>
              </dd>
            </dl>
      ...
      <p id="cart-info-txt" className="cart-info-txt-comment" />
      <div className="order-botton-area">
        {/* eslint-disable-next-line jsx-a11y/anchor-is-valid */}
        <a href="" className="botton-keep-shopping">
          계속 쇼핑하기
        </a>
        <button
          className="botton-solid"
