...
export default function MainCarousel() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:4080/api/best`).then(response => {
      const { data } = response;
      if (data) {
        const tmp: any[] = [[], []];
        data.data.forEach((item: any, index: number) => {
          if (index < 5) tmp[0].push(item);
    ...
    <Card>
      <Box width="12.5vw">
        <Link to={`/DtlPage/${item.productSeq}`}>
          <Box component="img" src={item.productImageUrl} width="100%" />
          <Typography m={1} fontWeight="900">
            {item.productTitle}
          </Typography>
          <Typography m={1} fontWeight="700" color="#999999">
            {item.productWriter}
          </Typography>
          <Typography m={1} fontWeight="900">
            {item.productPrice}원
